...
test = regedit
	...
	.on("data", function (entry) {
		...
		if (fs.existsSync(oculusAppAsar)) {
			console.log(
				'OcuTweaks is already installed. Run "npm run uninject" to uninstall OcuTweaks.'
			);
		} else {
			try {
				...
				fs.writeFileSync(
					...
					JSON.stringify({ name: "Oculus", main: "index.js", version: "0.0.0" })
				);
				...
				console.log(
					"OcuTweaks has been injected successfully! Restart the Oculus app for changes to take effect."
				);
			} catch (e) {
				console.log(e + "\n");
				console.log(
					'Failed to inject into Oculus app.\nPlease make sure to run "npm run inject" in an Administrator command prompt.'
				);
			}
		}
	});
